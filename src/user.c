...
bool userAdd(const char *filename, const char *ID, const SM2_KEY *sm2_key, const uint8_t key[ZUC_KEY_SIZE], const uint8_t iv[ZUC_KEY_SIZE])
{
    ...
    fwrite(public_key_info, sizeof(char), public_key_info_len, file); // 保存公钥
    ...
    fwrite(private_key_info, sizeof(char), private_key_info_len, file); // 保存私钥
...
bool userGet(const char *filename, const char *ID, SM2_KEY *sm2_key, uint8_t key[ZUC_KEY_SIZE], uint8_t iv[ZUC_KEY_SIZE])
{
    ...
    while (!feof(file))
    {
        ...
        {
            // 读取成功, 保存sm2公钥
            sm2_public_key_info_from_der(sm2_key, (const uint8_t **)&public_key_info_p, &public_key_info_len);

            // 读取成功, 保存sm2私钥
            sm2_private_key_info_from_der(sm2_key, (const uint8_t **)&private_key_info_p, &private_key_info_len, (const uint8_t **)&private_key_info_p, &private_key_info_len);
